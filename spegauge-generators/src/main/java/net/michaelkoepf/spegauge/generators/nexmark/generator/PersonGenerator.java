...
import static net.michaelkoepf.spegauge.generators.nexmark.generator.StringsGenerator.nextExtra;
import static net.michaelkoepf.spegauge.generators.nexmark.generator.StringsGenerator.nextString;
...
public class PersonGenerator {
   ...
   * batch of events.
   */
  private static final List<String> US_STATES = Arrays.asList("AZ,CA,ID,OR,WA".split(",")); // "WY"
  ...
  public static Person nextPerson(
          long nextEventId, UniformRandomProvider random, long timestamp, GeneratorConfig config) {
    ...
    return new Person(
        id, name, email, creditCard, city, state, Instant.ofEpochMilli(timestamp), extra);
  }

  /** Return a random person id (base 0). */
  public static long nextBase0PersonId(long eventId, UniformRandomProvider random, GeneratorConfig config) {
...
//    return Math.round(config.getNumActivePeople() * random.nextDouble());
  }

  /** Return a random person id (base 0). */
  public static long nextBase0PersonId(long eventId, Sampler sampler, GeneratorConfig config) {
    // Choose a random person from any of the 'active' people, plus a few 'leads'.
    // By limiting to 'active' we ensure the density of bids or auctions per person
    // does not decrease over time for long running jobs.
    // By choosing a person id ahead of the last valid person id we will make
    ...
    long numPeople = lastBase0PersonId(config, eventId) + 1;
    long activePeople = Math.min(numPeople, config.getNumActivePeople());
    long n = Math.round(sampler.sample() * (activePeople + PERSON_ID_LEAD));
    return numPeople - activePeople + n;
//    return Math.round(config.getNumActivePeople() * sampler.sample());
  }
  ...
  private static String nextUSState(UniformRandomProvider random) {
    return US_STATES.get(random.nextInt(US_STATES.size()));
  }

  /** Return a random US city. */
  private static String nextUSCity(UniformRandomProvider random) {
    return US_CITIES.get(random.nextInt(US_CITIES.size()));
  }

  /** Return a random person name. */
  private static String nextPersonName(UniformRandomProvider random) {
  ...
  private static String nextEmail(UniformRandomProvider random) {
    return nextString(random, 7) + "@" + nextString(random, 5) + ".com";
  }

  /** Return a random credit card number. */
  private static String nextCreditCard(UniformRandomProvider random) {
